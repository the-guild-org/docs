...
import { Container, Placeholder, Title, Wrapper } from './MarketplaceList.styles';
          ...
          {!pages[currentPage] || !pages[currentPage].length ? (
            ...
            <>
              <Table tableHeader={tableHeader ? tableHeader : <TableHeader />}
                     ...
                     pageCount={pageCount}
                     setCurrentPage={(page: { selected: number }) => setCurrentPage(page.selected)}
                     {...restProps.itemProps} />

              {currentItem && currentItem.modal && (
                <Modal {...currentItem.modal.header}
